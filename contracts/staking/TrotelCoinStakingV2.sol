...
contract TrotelCoinStakingV2 is AccessControl {
    ...
    modifier isValidDuration(uint32 duration) {
        require(rewards[duration].exists, "Invalid duration");
        _;
    }
    ...
    function increaseStaking(uint256 amount) external {
        ...
        userStaking.totalAmount = userStaking.totalAmount + amount;
    ...
    function unstake() external {
        ...
        for (uint256 i = 0; i < userStaking.amounts.length; i++) {
            uint256 stakingTime = userStaking.endTime - userStaking.times[i];
    ...
    function getVotingPower(address user) external view returns (uint256) {
        return stakings[user].totalAmount * multiplierVotingPower;
    }
